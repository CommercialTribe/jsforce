 ...
 */
Batch.prototype.result = function (jobId, batchId, resultId) {
	if (!jobId || !batchId) {
		throw new Error("Batch not started.");
	}
...
Bulk.prototype.query = function (soql, options) {
	...
	this.load(type, "query", options || {}, soql).then(function (results) {
		var lazyLoadedResultStreams = results.map(function (result) {
			...
			return function () {
				console.info('Starting to stream results from batch', result.batchId)
				return batch
					.result(result.id)
					.stream()
					.on('end', () => { console.info('Finished streaming results from batch', result.batchId) })
